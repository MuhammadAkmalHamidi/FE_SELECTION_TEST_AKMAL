...
export const Absen = () => {
    ...
    const getShift = async () => {
        try {
            const response = await Axios.get('http://localhost:2000/jadwal/check', {
        ...
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(async function (position) {
                ...
                if (distance < 25) {
                    ...
                    try {
                        const response = await Axios.post(`http://localhost:2000/absen/clockIn?shiftId=${shift}`, {}, {
                            headers: { Authorization: `Bearer ${token}` }
                        })
                        toast({
                            title: "Terima Kasih!",
    ...
    const clockOut = async () => {
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(async function (position) {
                ...
                if (distance < 10) {
                    try {
                        const response = await Axios.post(`http://localhost:2000/absen/clockOut?shiftId=${shift}`, {}, {
                            headers: { Authorization: `Bearer ${token}` }
                        })
                        toast({
                            title: "Terima kasih",

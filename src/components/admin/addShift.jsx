...
export const AddShift = ({ shiftId, reload, setReload }) => {
    ...
    const dataUser = async () => {
        try {
            const response = await Axios.get(`https://be-selection-test-akmal.vercel.app//user?page=1&filter=${roleId}&list=1`);
    ...
    const handleClick = async () => {
        try {
            const response = await Axios.post(`https://be-selection-test-akmal.vercel.app//jadwal/add/${userId}/${shiftId}`)
            setReload(!reload)
            toast({
                title: "Selamat!",
